...
```
Q1: ピース配置時のアニメーションはどの程度必要ですか？

1. なし（即座に配置）
   - メリット: 実装がシンプル、パフォーマンス良好
   - デメリット: 視覚的フィードバックが少ない

2. シンプル（フェードイン程度）
   - メリット: 適度なフィードバック、実装容易
   - デメリット: 差別化が弱い

3. リッチ（ドロップ＋バウンス）
   - メリット: ゲーム体験が向上
   - デメリット: 実装コスト高、パフォーマンス考慮必要

Q2: エラー発生時のユーザーへの通知方法は？
...
質問は上記の「質問の形式（必須）」に従う。バグ修正特有の質問例:

```
Q1: バグはどのタイミングで発生しますか？

1. 特定の操作を行うと必ず発生する
   - メリット: 再現が容易で原因特定しやすい
   - デメリット: なし

2. 特定の条件下でのみ発生する（ブラウザ、データ量など）
   - メリット: 条件を絞り込める
   - デメリット: 条件の特定作業が必要
